#!/usr/bin/env python
"""Train iTransformer for 2-day forecasting on 118-day training window."""
import numpy as np
import pandas as pd
import time
import warnings
...
print("\n" + "="*80)
print("STEP 2: ITRANSFORMER TRAINING - 118-DAY TRAINING + 2-DAY TEST")
print("="*80)
...
print(f"[OK] Train window: rows {train_start} to {train_end}")
print(f"     Training steps: {train_window} ({train_window/144:.1f} days)")
print(f"     Train dates: {train_df.index[0]} to {train_df.index[-1]}")
print(f"[OK] Test window: rows {test_start} to {len(df)}")
print(f"     Test steps: {horizon} ({horizon/144:.1f} days)")
...
print("\n" + "="*80)
print("RESULTS: 2-Day Forecast (trained on 118 days)")
print("="*80)
print(f"\nOverall Metrics:")
...
results_df.to_csv("itransformer_118day_results.csv", index=False)
metrics_df.to_csv("itransformer_118day_metrics.csv", index=False)

print(f"\n{'='*80}")
print("Files saved:")
print(f"  - itransformer_118day_results.csv")
print(f"  - itransformer_118day_metrics.csv")
print(f"{'='*80}\n")

"""Builds Marine_Forecast_RealEMS_31Param.ipynb -- the real-data (31-parameter,
1-min EMS simulation) counterpart to Marine_Forecast_PyTorch_NBEATS_NHiTS.ipynb.

Reuses the exact same model architectures (validated in the synthetic-data notebook),
just re-pointed at real data with: duplicate collapse (6 params), 3 circular angle
encodings, 10-min resampling, 2-day lookback / 48-hour horizon, and a new
precipitationType classifier.
"""
...
# =============================================================================
md(r"""# Marine 5-Day Forecast — Real 31-Parameter EMS Data (11-Model Comparison)
...
# =============================================================================
md(r"""## 1. Load real EMS data and inspect duplicate-collapse claims
...
# =============================================================================
md(r"""## 2. Separate categorical target, collapse duplicates, encode circular parameters
...
code(r"""df_cat = df_10min[["precipitationType"]].copy()
...
code(r"""CIRCULAR = ["windDirection", "currentDirection", "compass"]
for c in CIRCULAR:
...
# =============================================================================
md(r"""## 3. Train/test split, duplicate reconstruction fit, scaling
...
code(r"""# Fit duplicate reconstruction (slope, intercept) on TRAIN data only -- avoids any leakage
recon_coef = {}
for keep, drop in DUPLICATES:
...
# =============================================================================
md(r"""## 4. Shared training utilities (identical to the synthetic-data notebook)""")
code(r"""def train_model(model, X_tr, Y_tr, X_val, Y_val, epochs=150, batch_size=64,
...
# =============================================================================
md(r"""## 5. Model 1 — PyTorch LSTM (recursive rollout)""")
code(r"""class LSTMForecaster(nn.Module):
...
""")
code(r"""window = train_arr[-LOOKBACK:].copy()
lstm_preds_scaled = []
lstm_model.eval()
with torch.no_grad():
...
# =============================================================================
md(r"""## 6. Model 2 — XGBoost (direct multi-horizon, one model per parameter)
...
code(r"""ORIGIN_LAGS = [1, 2, 3, 6, 12, 24, 48, 72, 144, 288]   # in 10-min steps: 10min..48h
...
""")
code(r"""origin_idx = len(train_scaled) - 1
...
# =============================================================================
md(r"""## 7. Model 3 — N-BEATS (pure PyTorch)
...
code(r"""class NBeatsBlock(nn.Module):
...
# =============================================================================
md(r"""## 8. Model 4 — N-HiTS (pure PyTorch)
...
code(r"""class NHitsBlock(nn.Module):
...
# =============================================================================
md(r"""## 9. Model 5 — DLinear (Zeng et al., AAAI 2023)""")
code(r"""class MovingAvg(nn.Module):
...
# =============================================================================
md(r"""## 10. Model 6 — TiDE (Das et al., Google, TMLR 2023)""")
...
# =============================================================================
md(r"""## 11. Model 7 — TSMixer (Chen et al., Google, 2023)
...
code(r"""class TSMixerBlock(nn.Module):
...
# =============================================================================
md(r"""## 12. Model 8 — Harmonic Regression + Residual MLP Hybrid
...
code(r"""PERIODS_STEPS = [24.0 * 6, 12.0 * 6, 12.42 * 6, 23.93 * 6, 25.82 * 6, 6.0 * 24 * 6]
...
""")
code(r"""resid_mean = residual_train.mean()
...
# =============================================================================
md(r"""## 13. Model 9 — iTransformer (Liu et al., 2023)
...
# =============================================================================
md(r"""## 14. Model 10 — PatchTST (Nie et al., ICLR 2023)
...
code(r"""class PatchTST(nn.Module):
...
# =============================================================================
md(r"""## 15. Model 11 — DeepAR (Salinas et al., Amazon)
...
# =============================================================================
md(r"""## 16. Reconstruct physical values & score all 11 models
...
code(r"""def reconstruct(pred_df):
    out = pred_df.copy()
...
# =============================================================================
md(r"""## 17. Visualize the validation""")
code(r"""hist_tail = df_num_full.iloc[-HORIZON - LOOKBACK:-HORIZON]
...
# =============================================================================
md(r"""## 18. Save outputs for the results dashboard""")
...
# =============================================================================
md(r"""## 19. Best model per parameter (ranked)""")
code(r"""def best_skill_value(row):
...
# =============================================================================
md(r"""## 20. Two-model ensembles — can combining models beat the single best?
...
code(r"""from itertools import combinations
...
ensemble_df
""")
code(r"""ens_fva = pd.DataFrame({"timestamp": truth.index})
for p in report_params:
...
# =============================================================================
md(r"""## 21. PrecipitationType classifier (XGBoost multiclass)
...
code(r"""from sklearn.preprocessing import LabelEncoder
...
""")
code(r"""origin_idx_c = len(train_scaled) - 1
...
# =============================================================================
md(r"""## 22. Duplicate reconstruction sanity check
...
code(r"""print("Duplicate reconstruction error on the held-out forecast (using each model's own prediction):")
for keep, drop in DUPLICATES:
...
# =============================================================================
md(r"""## 23. How to read the results
...
nb["cells"] = cells
with open("Marine_Forecast_RealEMS_31Param.ipynb", "w", encoding="utf-8") as f:
    nbf.write(nb, f)
print("Final notebook written:", len(cells), "cells")

#!/usr/bin/env python3
"""Marine iTransformer training pipeline."""
...
from portland_itransformer.preprocessing_marine import MarinePreprocessor
...
from portland_itransformer.preprocessing_marine import TARGET_MARINE, KNOWN_FEATURES_MARINE
...
def main():
    """Run Marine iTransformer training."""

    logger.info("=" * 100)
    logger.info("MARINE iTRANSFORMER TRAINING PIPELINE")
    logger.info("=" * 100)

    # Load config
    logger.info("\n[1/6] Loading configuration...")
    config_path = project_root / "configs" / "marine_24h.yaml"
    ...
    # Preprocess
    logger.info("\n[2/6] Preprocessing marine data...")
    preprocessor = MarinePreprocessor(config)
    ...
    df = pd.read_parquet(Path(config.paths.processed_dir) / "marine_preprocessed.parquet")
    split_labels = np.load(Path(config.paths.processed_dir) / "marine_split_labels.npy")
    ...
    loaders = create_data_loaders(
        ...
        target_cols=TARGET_MARINE,
        known_cols=KNOWN_FEATURES_MARINE,
    )
    ...
    target_loss_weights = {
        f'target_{i}': marine_weights.get(TARGET_MARINE[i], 1.0)
        for i in range(len(TARGET_MARINE))
    }

    model = MarineITransformer(
        seq_len=config.data.seq_len,
        pred_len=config.data.pred_len,
        n_input_features=len(TARGET_MARINE) + len(KNOWN_FEATURES_MARINE),
        n_target_features=len(TARGET_MARINE),
        n_future_known=len(KNOWN_FEATURES_MARINE),
    ...
    import joblib
    scaler_targets = joblib.load(Path(config.paths.artifacts_dir) / "marine_scaler_targets.joblib")
    ...
    logger.info("\n" + "=" * 100)
    logger.info("MARINE iTRANSFORMER FINAL RESULTS")
    logger.info("=" * 100)
    ...
    logger.info("\n✅ Marine iTransformer training complete!")
